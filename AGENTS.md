...
- After token validation, Session limits IDENTIFY by user ID and authenticator Session ID. Redis atomically claims one live logical Session per authenticator Session and renews the claim throughout the detached resume window. Claims include the owning Session node ID and generation; IDENTIFY may CAS-take over an existing claim only after etcd confirms that exact generation is gone.
