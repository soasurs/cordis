...
- `services/message/v1`: go-zero `zrpc` on `:3002`; messages, mentions, and 1:1 DM channels in Postgres, publishes message events directly to Kafka when Kafka is configured, and calls User gRPC for relationship checks on DM flows. `GetReadStates` bounds Guild authorization fan-out and acquires process-local weighted capacity based on the deduplicated channel count.
