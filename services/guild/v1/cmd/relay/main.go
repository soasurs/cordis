...
func main() {
	...
	guildRelay, err := relay.New(relay.Config{
		...
		Publisher:     publisher,
		Namespace:     eventoutbox.GuildAdvisoryNamespace,
		NotifyChannel: notifyChannel(cfg.Relay.NotifyEnabled, eventoutbox.GuildNotifyChannel),
	...
	workers.Go(func() {
		if err := guildRelay.Run(ctx); err != nil && ctx.Err() == nil {
			logx.Errorw("guild outbox relay stopped", logx.Field("error", err))
		}
	})

	logx.Infow("starting guild outbox relay",
		logx.Field("workers", cfg.Relay.Workers),
		logx.Field("topic", cfg.Kafka.Topic),
	)
	<-ctx.Done()
	logx.Info("stopping guild outbox relay")
	workers.Wait()
}
