...
func TestLoadConfig(t *testing.T) {
	...
	require.Equal(t, 40*time.Second, cfg.Node.ShutdownTimeout())
	require.Equal(t, int64(120), cfg.RateLimit.Policies["identify_user"].Limit)
	require.Equal(t, time.Minute, cfg.RateLimit.Policies["identify_user"].Window)
	require.Equal(t, int64(60), cfg.RateLimit.Policies["identify_auth_session"].Limit)
	require.Equal(t, time.Minute, cfg.RateLimit.Policies["identify_auth_session"].Window)
