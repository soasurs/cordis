...
- Session visibility loading is bounded to 100 Guilds and 500 visible channels per Guild by default. Malformed, oversized, missing, or explicitly invalid snapshots fail closed. Message delivery remains on channel subscriptions until revision-bearing invalidation/rebuild is complete.
- Dispatcher resolves Guild `message.*` through aggregate Guild routes, then uses the existing channel dispatch RPC so Session still filters by local channel subscriptions. DM `message.*` uses aggregate user routes. Legacy records without aggregate route IDs continue through channel routes during migration.
