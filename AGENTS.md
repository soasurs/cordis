...
- `services/user/v1`: go-zero `zrpc` on `:3000`; users/profiles in Postgres, Argon2id password hashing. `users.email_verified_at` tracks verification; `UpdateEmail` clears it, `MarkEmailVerified` only applies while the supplied email is still current, and internal-only `ResetPassword` replaces the password without the old one.
