...
- `services/presence/v1`: gRPC on `:3003`; Redis-backed gateway liveness, channel routing, and user presence TTLs. Publishes `presence.updated` to its own optional Kafka topic (default `cordis.presence.events.v1`) only when the multi-device aggregate status actually transitions; heartbeat renewals stay silent. The pre/post aggregate snapshot happens in the server layer, and a failed pre-read skips the event rather than guessing.
