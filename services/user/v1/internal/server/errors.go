...
func isUsernameViolation(err error) bool {
	var pgErr *pgconn.PgError
	return errors.As(err, &pgErr) && pgErr.Code == "23505" && pgErr.ConstraintName == "user_profiles_username_active_idx"
}
...
var (
	...
	errBioTooLong           = status.Error(codes.InvalidArgument, "bio is too long")
	errUpdateFieldsRequired = status.Error(codes.InvalidArgument, "at least one profile field is required")
	errAvatarAssetIDInvalid = status.Error(codes.InvalidArgument, "avatar asset id must be non-negative")
