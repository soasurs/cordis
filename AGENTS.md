...
- `services/user/v1`: go-zero `zrpc` on `:3000`; user identity and profiles in Postgres, no credential material. Emails are lowercased at every entry point (a CHECK constraint backstops storage). `users.email_verified_at` tracks verification; `UpdateEmail` clears it only when the address actually changes and `MarkEmailVerified` only applies while the supplied email is still current.
