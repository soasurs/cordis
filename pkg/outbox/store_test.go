...
func deletePattern() string {
	return sqlPattern(`DELETE FROM outbox_messages WHERE id = $1`)
}
...
func TestInsert(t *testing.T) {
	...
	mock.ExpectExec(insertPattern()).
		WithArgs(int64(1), "test.topic", []byte("key-1"), []byte(payload), 0, 5, int64(0), sqlmock.AnyArg()).
...
func TestInsertNilKey(t *testing.T) {
	...
	mock.ExpectExec(insertPattern()).
		WithArgs(int64(2), "test.topic", sqlmock.AnyArg(), []byte(payload), 0, 5, int64(0), sqlmock.AnyArg()).
		WillReturnResult(sqlmock.NewResult(0, 1))
...
func TestClaimOne(t *testing.T) {
	...
	// Exact query generated by ClaimOne(_, _, id=1, now=X)
	claimOneQuery := `UPDATE outbox_messages SET locked_at = $1 ` +
		`WHERE id IN ( SELECT id FROM outbox_messages WHERE locked_at = 0 AND id = $2 ` +
	...
	mock.ExpectQuery(sqlPattern(claimOneQuery)).
		WithArgs(sqlmock.AnyArg(), int64(1), 1).
		WillReturnRows(rows)

	evt, err := ClaimOne(context.Background(), db, 1, Now())
	if err != nil {
		t.Fatalf("ClaimOne returned error: %v", err)
	}
	if evt == nil {
		t.Fatal("ClaimOne returned nil event")
	}
	if evt.ID != 1 || evt.Topic != "test.topic" || string(evt.Key) != "key-1" {
		t.Fatalf("unexpected event: %+v", evt)
	}
}

func TestClaimOneAlreadyClaimed(t *testing.T) {
	db, mock, cleanup := newTestDB(t)
	defer cleanup()
	...
	if evt != nil {
		t.Fatalf("expected nil event when already claimed, got %+v", evt)
	}
}
...
func TestClaimBatch(t *testing.T) {
	...
	// Exact query generated by ClaimBatch(_, _, now=X, limit=10)
...
func TestDelete(t *testing.T) {
	...
	if err := Delete(context.Background(), db, 1); err != nil {
