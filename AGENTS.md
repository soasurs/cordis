...
- `services/mailer/v1`: go-zero `zrpc` on `:3007`; stateless transactional mail delivery behind a `SendEmail(to, template, variables)` RPC. Template names and variable keys live in `pkg/mail`. Providers are `noop` (redacted logging, no delivery) and SMTP with optional authentication and STARTTLS; the local Compose stack sends through Mailpit. SMTP templates render configured password-reset and email-verification URLs with the secret token in the query string. Template variables may contain secrets and must never be logged verbatim.
