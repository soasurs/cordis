...
- OpenTelemetry：RPC 服务可通过 `CORDIS_OTEL_ENDPOINT` 输出 trace。`pkg/database.NewPostgres` 使用 `github.com/XSAM/otelsql` 包装 Postgres 连接，带 request context 的 Store SQL 会作为当前 RPC span 的子 span。
